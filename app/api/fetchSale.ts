...
export async function GET(req: Request) {
  ...
  const baseOlxUrl = `https://www.olx.ro/imobiliare/apartamente-garsoniere-de-vanzare/q-${encodeURIComponent(name)}/?currency=RON`;
  ...
  try {
    ...
    return NextResponse.json({ sale: listings });
  } catch (error) {
    return NextResponse.json({ sale: [], error: error instanceof Error ? error.message : "Unknown error" }, { status: 500 });
  } finally {
